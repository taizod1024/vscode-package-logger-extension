...
try {
  ...
  Write-Host -ForegroundColor Yellow "[$(timestamp)] $($app_name)"
  Write-Host -ForegroundColor Green "[$(timestamp)] - logPath: $($logPath)"
  Write-Host -ForegroundColor Green "[$(timestamp)] - tmpPath: $($tmpPath)"
  Write-Host -ForegroundColor Green "[$(timestamp)] - isUpdate: $($isUpdate)"

  # clear temporary path
  Write-Host -ForegroundColor Green "[$(timestamp)] - remove tmpPath"
  if (Test-Path $tmpPath) { Remove-Item $tmpPath -Recurse -Force }
  Write-Host -ForegroundColor Green "[$(timestamp)] - create tmpPath"
  ...
  if ($isUpdate) {
    ...
    # windows update
    Write-Host -ForegroundColor Green "[$(timestamp)] - update os"
    Get-Command abc-update | Out-Null
    if (-not $?) {
      Write-Host -ForegroundColor Yellow "[$(timestamp)]   => abc-update not found"
    }
    else {
    ...
    # update chocolatey
    Write-Host -ForegroundColor Green "[$(timestamp)] - update chocolatey"
    Get-Command choco | Out-Null
    if (-not $?) {
      Write-Host -ForegroundColor Yellow "[$(timestamp)]   => choco not found"
    }
    else {
    ...
    # update nodejs
    Write-Host -ForegroundColor Green "[$(timestamp)] - update nodejs"
    Get-Command npm | Out-Null
    if (-not $?) {
      Write-Host -ForegroundColor Yellow "[$(timestamp)]   => npm not found"
    }
    else {
      npm update -g
    }
    ...
    # update vscode
    Write-Host -ForegroundColor Green "[$(timestamp)] - update vscode"
    Get-Command code | Out-Null
    if (-not $?) {
      Write-Host -ForegroundColor Yellow "[$(timestamp)]   => vscode not found"
    }
    else {
  ...
  function Invoke-ScriptAt() {
    ...
    Write-Host -ForegroundColor Green "[$(timestamp)] - $($path)"
  ...
  Invoke-ScriptAt "os/env" {
    Get-ChildItem env: | ForEach-Object {
      ...
      if ($_.Name -eq "Path" -or $_.Name -eq "PATHEXT") {
        ...
        Write-Host -ForegroundColor Yellow "[$(timestamp)]   - $($text)"
      }
  ...
  # success
  Write-Host -ForegroundColor Green "[$(timestamp)] - move tmpPath to logPath"
  ...
  if ($vscode_process.Count -eq 0) {
    ...
    Write-Host -ForegroundColor Green "[$(timestamp)] - done"
    timeout 5
  }
